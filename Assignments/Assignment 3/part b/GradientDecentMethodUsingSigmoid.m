function Weights = GradientDecentMethodUsingSigmoid(Weights,TrainingImages,CorrectLabels)
...
for i=1:size(TrainingImages,1)
    ...
    CalculatedLabel=SigmoidActivationFunction(weightedSum);
    %We can view it as yi* of the mathematical equation
    
    error=CurrentCorrectLabel-CalculatedLabel;
    
    delta=CalculatedLabel*(1-CalculatedLabel)*error; %d(x)=(x)(1-x).
